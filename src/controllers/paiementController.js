...
const payerMoovMoney = asyncHandler(async (req, res) => {
  ...
  try {
    // FIX: customerPayMerchant() retourne `transactionId`, pas `referenceId`
    // (l'ancien code déstructurait un champ qui n'existe pas -> toujours undefined).
    const { transactionId } = await moovMoney.customerPayMerchant({
      telephone: numero_moov,
      amount: MONTANT,
      reference: idTransaction,
      message: 'Abonnement Mali Kalan — Découverte',
...
const verifierStatutPaiement = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // FIX: on loggue désormais l'erreur côté serveur (Railway) au lieu
    // de la laisser invisible. Sans ça, une vraie panne de l'appel
    // fournisseur (Orange/Moov) était indiscernable d'une simple attente
    // normale côté app, qui recevait juste { statut: 'En attente' }.
    console.error(
      `❌ Erreur polling statut (${idTransaction}):`, err.message
    )
    // Le fournisseur n'a pas répondu — on reste en attente, l'app réessaiera
    res.json({ statut: 'En attente', warning: err.message })
  }
